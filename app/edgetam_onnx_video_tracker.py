#!/usr/bin/env python3
"""EdgeTAM — ONNX Runtime Video Tracker

Single-object tracking: give a bounding box or point on frame 0, track forever.
Uses ONNX Runtime + CUDA for inference.

Adapted from SAM2.1 ONNX tracker for EdgeTAM architecture:
- Spatial Perceiver compresses memory: [B, 64, 64, 64] -> [B, 512, 64]
- No temporal positional encoding on object pointers
- mem_dim=64, obj_ptrs split into 4 tokens each (256/64)
"""
...
IMG_SIZE = 1024        # EdgeTAM input resolution
MEM_TOKENS = 512       # Spatial perceiver output tokens per mask (256 1D + 256 2D)
MEM_DIM = 64           # Memory feature dimension
HIDDEN_DIM = 256       # Model hidden dimension
OBJ_PTR_SPLIT = HIDDEN_DIM // MEM_DIM  # 4 tokens per object pointer
...
@dataclass
class MemoryFrame:
    maskmem_features: np.ndarray  # [1, 512, 64] (perceiver output)
    maskmem_pos_enc: np.ndarray   # [1, 512, 64]
...
class MemoryBank:
    ...
    def build_memory_inputs(self, current_idx: int):
        """Build the inputs for the memory_attention ONNX model.

        EdgeTAM differences from SAM2:
        - memory_1 is [1, num_masks, 512, 64] (perceiver output, not spatial 64x64)
        - memory_pos_embed has 512 tokens per mask (not 4096)
        - No temporal pos encoding on obj ptrs (obj_pos is zeros)
        - Input is num_obj_ptr (int64) instead of cond_frame_id_diff (float)
        """
        ...
        num_masks = len(frames_tpos)

        # memory_1: [1, num_masks, 512, 64] (perceiver-compressed spatial memory)
        memory_1 = np.stack(
        ...
        for f, t_pos in frames_tpos:
            ...
            tcode = self.temporal_code[tcode_idx]  # [1, 1, 64]
            tcode = tcode.reshape(1, 1, MEM_DIM)
            pos = f.maskmem_pos_enc + tcode  # [1, 512, 64] + broadcast
            pos_parts.append(pos[0])  # [512, 64]
        mask_pos = np.concatenate(pos_parts, axis=0)  # [num_masks*512, 64]

        # memory_0: [1, num_obj_ptr, 256] (object pointers)
        # EdgeTAM uses only_obj_ptrs_in_the_past_for_eval=True
        ...
        # memory_pos_embed: [1, num_masks*512 + 4*num_obj_ptr, 64]
        # EdgeTAM: add_tpos_enc_to_obj_ptrs=False, so obj_pos is zeros
        obj_pos_zeros = np.zeros(
            (num_obj_ptr * OBJ_PTR_SPLIT, MEM_DIM), dtype=np.float32
        )
        memory_pos_embed = np.concatenate(
            [mask_pos, obj_pos_zeros], axis=0
        )[np.newaxis].astype(np.float32)

        # num_obj_ptr as int64 scalar
        num_obj_ptr_tensor = np.array(num_obj_ptr, dtype=np.int64)

        return memory_0, memory_1, memory_pos_embed, num_obj_ptr_tensor
...
def main():
    ap = argparse.ArgumentParser(description="EdgeTAM ONNX Runtime Video Tracker")
    ...
    ap.add_argument("--model-dir", default="/models/edgetam/onnx/",
    ...
    ap.add_argument("--save-masks", default=None,
                    help="Directory to save per-frame binary masks as .npz")
    ...
    # ── Load ONNX sessions ──────────────────────────────────────────────
    trt_cache = os.path.join(args.model_dir, "trt_cache")
    ...
    # Mask decoder has dynamic shapes that TensorRT can't handle (If nodes,
    # dynamic ConvTranspose), so always use CUDA for it
    cuda_providers = [("CUDAExecutionProvider", {}), "CPUExecutionProvider"]
    print("Loading ONNX models...")
    enc_sess  = make_session(os.path.join(args.model_dir, "image_encoder.onnx"), providers)
    dec_sess  = make_session(os.path.join(args.model_dir, "mask_decoder.onnx"), cuda_providers)
    ...
    bank = MemoryBank()
    frame_idx = 0
    menc_input_names = [i.name for i in menc_sess.get_inputs()]
    matt_input_names = [i.name for i in matt_sess.get_inputs()]
    ...
    while True:
        ...
        if frame_idx == 0:
            ...
            menc_inputs = {"mask_for_mem": mask_for_mem, "pix_feat": pix_feat}
            if "occ_logit" in menc_input_names:
                menc_inputs["occ_logit"] = occ_logit
            ...
            mem0, mem1, mem_pos, _ = bank.build_memory_inputs(frame_idx)
            ...
            # num_obj_ptr may be folded as constant during export
            matt_input_names = [i.name for i in matt_sess.get_inputs()]
            if "num_obj_ptr" in matt_input_names:
                matt_inputs["num_obj_ptr"] = np.array(mem0.shape[1], dtype=np.int64)
            image_embed = matt_sess.run(None, matt_inputs)[0]

            # No prompt for tracking frames (label -1 = padding)
            ...
            menc_inputs2 = {"mask_for_mem": mask_for_mem, "pix_feat": pix_feat}
            if "occ_logit" in menc_input_names:
                menc_inputs2["occ_logit"] = occ_logit
        ...
        if args.show:
            cv2.imshow("EdgeTAM Tracker", vis)
