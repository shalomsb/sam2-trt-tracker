#!/usr/bin/env python3
"""EdgeTAM — Full TensorRT Video Tracker (all 4 engines as TRT)."""
...
NUM_MASKMEM = 7        # Must match EdgeTAM's temporal_code size (7 entries)
IMG_SIZE = 1024
MEM_TOKENS = 512       # Spatial perceiver output tokens per mask
...
@dataclass
class MemoryFrame:
    maskmem_features: torch.Tensor  # [1, 512, 64] (perceiver output)
    maskmem_pos_enc: torch.Tensor   # [1, 512, 64]
    obj_ptr: torch.Tensor           # [1, 256]
    frame_idx: int
    is_cond: bool
...
class MemoryBank:
    def __init__(self):
        ...
        max_pos = NUM_MASKMEM * MEM_TOKENS + NUM_MASKMEM * OBJ_PTR_SPLIT
        self._mem1 = torch.empty(1, NUM_MASKMEM, MEM_TOKENS, MEM_DIM, device="cuda")
        self._mem0 = torch.empty(1, NUM_MASKMEM, HIDDEN_DIM, device="cuda")
        self._mempos = torch.zeros(1, max_pos, MEM_DIM, device="cuda")
    ...
    def build_memory_inputs(self, current_idx: int):
        ...
        # memory_1 — write perceiver-compressed features into pre-allocated slots
        ...
        for i, (f, t_pos) in enumerate(frames_tpos):
            tcode = self.temporal_code[NUM_MASKMEM - 1 - t_pos].view(1, MEM_DIM)
            s = i * MEM_TOKENS
            self._mempos[0, s:s + MEM_TOKENS].copy_(f.maskmem_pos_enc[0])
            self._mempos[0, s:s + MEM_TOKENS].add_(tcode)
        ...
        # Zero the obj_ptr portion of mempos (EdgeTAM: add_tpos_enc=false)
        total_pos = n * MEM_TOKENS + ptr_n * OBJ_PTR_SPLIT
        self._mempos[0, n * MEM_TOKENS:total_pos].zero_()
...
def main():
    ap = argparse.ArgumentParser(description="EdgeTAM TRT Tracker (GPU-optimised)")
    ap.add_argument("--video", required=True)
    ap.add_argument("--bbox", help="x1,y1,x2,y2")
    ap.add_argument("--point", help="x,y")
    ap.add_argument("--model-dir", default="/models/edgetam/trt/")
    ap.add_argument("--output", default="/output/output_tracked_full_trt.mp4")
    ...
    # ── Load TRT engines (all on shared stream) ─────────────────────────
    ...
    # Detect which inputs each engine expects (some may be folded as constants)
    menc_has_occ = "occ_logit" in menc.input_names
    matt_has_num_obj = "num_obj_ptr" in matt.input_names
    ...
    menc_warmup = {"mask_for_mem": torch.zeros(1, 1, 1024, 1024, device="cuda"),
                   "pix_feat": pf}
    if menc_has_occ:
        menc_warmup["occ_logit"] = torch.zeros(1, 1, device="cuda")
    menc(menc_warmup)

    matt_warmup = {"current_vision_feat": vf, "current_vision_pos_embed": vp,
                   "memory_0": torch.zeros(1, 1, 256, device="cuda"),
                   "memory_1": torch.zeros(1, 1, MEM_TOKENS, MEM_DIM, device="cuda"),
                   "memory_pos_embed": torch.zeros(1, MEM_TOKENS + OBJ_PTR_SPLIT, MEM_DIM, device="cuda")}
    if matt_has_num_obj:
        matt_warmup["num_obj_ptr"] = torch.tensor(1, dtype=torch.int64, device="cuda")
    matt(matt_warmup)
    ...
    while True:
        ...
        if frame_idx == 0:
            ...
            maskmem_feat, maskmem_pos, temporal_code = menc(menc_inputs)
            ...
            mem0, mem1, mem_pos = bank.build_memory_inputs(frame_idx)
            ...
            if matt_has_num_obj:
                matt_inputs["num_obj_ptr"] = torch.tensor(
                    mem0.shape[1], dtype=torch.int64, device="cuda"
                )
            (image_embed,) = matt(matt_inputs)
            ...
            if menc_has_occ:
                menc_inputs["occ_logit"] = occ_logit
            maskmem_feat, maskmem_pos, _ = menc(menc_inputs)
            e_menc_e.record(trt_stream)
